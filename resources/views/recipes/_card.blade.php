...
<div class="card h-100 shadow-sm">
    ...
    <div style="position: relative; overflow: hidden; border-radius: calc(0.375rem - 1px) calc(0.375rem - 1px) 0 0;">
        @if($image)
                            <img src="{{ Str::startsWith($image, ['http://', 'https://']) ? $image : Storage::url($image) }}"
                                    class="card-img-top" alt="{{ data_get($recipe, 'title') ? 'Photo of ' . data_get($recipe, 'title') : 'Recipe photo' }}"
                                    ...
                                    onerror="if(this.parentNode){
                                        ...
                                        fallback.innerHTML = '<i class=\'fa-regular fa-image text-muted\' style=\'font-size: 3rem; opacity: 0.5;\' aria-hidden=\'true\'></i>';
        ...
        @else
            <div class="card-img-top d-flex align-items-center justify-content-center bg-secondary bg-opacity-25" style="height: 200px;">
                <i class="fa-regular fa-image text-muted" style="font-size: 3rem; opacity: 0.5;" aria-hidden="true"></i>
            </div>
        @endif
        ...
        @if($isOwnedByUser && data_get($recipe, 'status') === 'pending')
            <div class="position-absolute top-0 start-0 m-2" style="z-index: 10;">
                <span class="badge bg-warning text-dark shadow-sm" data-bs-toggle="tooltip" data-bs-title="Awaiting moderation" style="backdrop-filter: blur(8px);">
                    <i class="fa-solid fa-clock me-1" aria-hidden="true"></i> Pending
                </span>
            </div>
        ...
        @if($isOwnedByUser && data_get($recipe, 'is_public') !== null)
            <div class="position-absolute top-0 end-0 m-2" style="z-index: 10;">
                ...
                <span class="badge {{ $isPublic ? 'bg-success' : 'bg-secondary' }} shadow-sm" data-bs-toggle="tooltip" data-bs-title="{{ $visibilityLabel }}" style="backdrop-filter: blur(8px); {{ $isPublic ? 'background: rgba(25, 135, 84, 0.9) !important;' : 'background: rgba(108, 117, 125, 0.9) !important;' }}">
                    <i class="fa-solid fa-{{ data_get($recipe, 'is_public') ? 'globe' : 'lock' }}" aria-hidden="true"></i>
                </span>
            </div>
        @endif
    </div>
    ...
    <div class="card-body d-flex flex-column position-relative">
        ...
        <p class="text-muted mb-2 small" style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
            ...
            @endif
            @if($displayTime)
                <i class="fa-regular fa-clock" style="font-size: 0.75rem;" aria-hidden="true"></i> {{ $displayTime }}
            @endif
            @if($recipe->serves)
                <span class="mx-1">·</span>
                <span data-bs-toggle="tooltip" title="Serves {{ e($recipe->serves) }}">
                    <i class="fa-solid fa-user-group" style="font-size: 0.75rem;" aria-hidden="true"></i> {{ $recipe->serves }}
                </span>
            @endif
                ...
                <span style="letter-spacing: 1px;">
                    @if($recipe->difficulty === 'easy')
                        <span style="color: #28a745;" data-bs-toggle="tooltip" data-bs-title="Easy difficulty">
                            <i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-regular fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-regular fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i>
                        </span>
                    @elseif($recipe->difficulty === 'medium')
                        <span style="color: #ffc107;" data-bs-toggle="tooltip" data-bs-title="Medium difficulty">
                            <i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-regular fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i>
                        </span>
                    @else
                        <span style="color: #dc3545;" data-bs-toggle="tooltip" data-bs-title="Hard difficulty">
                            <i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i><i class="fa-solid fa-circle" style="font-size: 0.45rem;" aria-hidden="true"></i>
                        </span>
                    @endif
                </span>
            @endif
        ...
        <div class="mb-3">
            @if($avgRating)
                <div class="text-warning small d-flex align-items-center gap-1">
                    <span>
                        @for($i = 1; $i <= 5; $i++)
                            <i class="fa-{{ $i <= round($avgRating) ? 'solid' : 'regular' }} fa-star" style="font-size: 0.875rem;" aria-hidden="true"></i>
                        @endfor
                    </span>
            ...
            @else
                <div class="text-muted small" style="opacity: 0.7;">
                    <i class="fa-regular fa-star" aria-hidden="true"></i> No ratings yet
                </div>
            @endif
        </div>
